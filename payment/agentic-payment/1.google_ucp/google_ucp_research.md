...
UCP 的 Checkout 能力是整个协议最核心的交易执行层。UCP 提供了两种结账机制，分别面向不同的商户需求和安全模型。在深入技术细节之前，先理解两者的定位和设计思路。

#### 两种结账机制概要

**为什么需要两种机制？**

核心矛盾在于：AI Agent 购物需要标准化（降低集成复杂度），但商户需要差异化（保留品牌体验和自定义逻辑）。一种机制无法同时满足这两个需求，因此 UCP 设计了两条路径：

| 维度 | Native Checkout（原生结账） | Embedded Checkout（嵌入式结账） |
|------|--------------------------|-------------------------------|
| 一句话定义 | 商户提供 REST API，Google 平台调用 API 并在自己的 UI 中完成结账 | 商户提供 Web 结账页面，通过 iframe 嵌入 Google 界面，双方通过 JSON-RPC 通信 |
| 核心设计思路 | **平台主导**——Google 控制整个结账 UI，商户只负责后端逻辑 | **商户主导**——商户控制结账 UI，Google 只提供地址和支付凭证 |
| 解决的问题 | 标准商品的快速标准化接入，降低商户开发成本 | 复杂商品（定制、配置、捆绑）需要商户特有的交互逻辑 |
| 敏感信息处理 | 配送地址、支付方式等敏感信息完全由 Google UI（Trusted UI）处理，Agent 不可触及 | 商户可以选择委托给 Google 处理，也可以在自己的 iframe 中处理 |
| 品牌体验 | 统一的 Google 风格，商户品牌弱化 | 商户保留完整品牌视觉、交叉销售机会 |
| 典型商户 | 标准零售（服装、电子产品、日用品） | 高度定制化商户（珠宝定制、家具配置、旅行套餐） |

**设计思路的本质区别**：

```
Native Checkout：API 驱动模型
┌─────────────────────────────────────────┐
│  Google 平台（控制 UI + 调用 API）        │
│  ┌───────────────────────────────────┐  │
│  │  Agent：搜索商品、构建购物车        │  │
│  │  （非敏感操作）                     │  │
│  ├───────────────────────────────────┤  │
│  │  Google UI：地址、支付、确认购买    │  │
│  │  （敏感操作，Trusted UI）           │  │
│  └───────────────────────────────────┘  │
│           ↕ REST API 调用                │
│  ┌───────────────────────────────────┐  │
...
- 如果只有 Native Checkout：商户失去品牌控制力，复杂商品（如需要用户选择尺寸、颜色、刻字内容的珠宝）无法在标准 API 中表达所有交互逻辑
- 如果只有 Embedded Checkout：每个商户都要构建嵌入式 Web UI + JSON-RPC 通信，开发成本高，且用户体验因商户而异，无法保证一致性
- 两种机制互补：80% 的标准商品用 Native（低成本、高一致性），20% 的复杂商品用 Embedded（高灵活性、保留品牌）
