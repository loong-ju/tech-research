...
- **AWS**：暂不展开（地基层）
- 关联：`_foundation/00-foundation-business.md` + `00-foundation-tech.md`
- 状态：✅ 已生成（业务篇+技术篇）
...
- 关联：`card-payment/01-cards-business.md` + `01-cards-tech-aws.md`
- 状态：✅ 已生成（业务篇+技术篇）
...
- **技术篇**：支付网关架构、异步通知/回调、支付状态机、对账系统、幂等设计、钱包账本、聚合路由、高并发削峰、令牌化(tokenization)
  - **AWS**：API Gateway + Step Functions + SQS/EventBridge 异步、DynamoDB 幂等、Aurora 账本、Glue 对账、高可用多区
- 关联：`e-payment/02-epayment-business.md` + `02-epayment-tech-aws.md`
- 状态：✅ 已生成（业务篇+技术篇）

### 模块 3 · 跨境支付（重点）
> 你已有基础（card-payment/e-payment/crossborder-payment-payment 目录）。核心：跨境没有共同账本。
...
- 关联：`03-crossborder-business.md`（业务总入口，已整合四套管道骨架+G20目标+新兴技术+引用来源）+ `03-crossborder-tech-aws.md`（技术）+ `03b`（收款全链路）+ `03c-crossborder-players/`（企业画像）
- 状态：✅ 已生成（业务篇+技术篇，配合已有深度报告）
...
- 关联：`04-stablecoin-business.md` + `04-stablecoin-tech-aws.md`（教学篇）+ 已有 `stablecoin_research.md`/`stablecoin_cross_border_compliance.md`/`LEARNING_NOTES`（深度参考）
- 状态：✅ 已生成（业务篇+技术篇，配合已有研究）
...
- 关联：`05-agentic-payment-business.md` + `05-agentic-payment-tech-aws.md`（教学整合骨架）+ 已有 8 个协议专题 + 综述 `agentic_commerce.md` + 风控 `10.fraud_risk_control/` + `reference/summary/`
- 状态：✅ 已生成（业务篇+技术篇，整合已有专题）
...
- 关联：`_topics/61-risk-fraud.md`、`62-compliance.md`、`63-ledger-reconciliation.md`、`64-non-functional.md`
- 状态：✅ 已生成（4 个专题）
...
```
_foundation/        模块0 地基(业务/技术)
card-payment/       模块1 银行卡
  01-cards-business.md           模块1 银行卡(业务)
  01-cards-tech-aws.md           模块1 银行卡(技术+AWS)
  01c-hsm-payment-cryptography.md  HSM/支付密码学
e-payment/           模块2 电子支付
  02-epayment-business.md        模块2 电子支付(业务)
  02-epayment-tech-aws.md        模块2 电子支付(技术+AWS)
  02b-ecommerce-payment.md       电商支付业务场景(买家付/卖家收)
  02c-epayment-players/          电子支付/收单企业画像
crossborder-payment/        模块3 跨境支付
  03-crossborder-business.md     模块3 跨境(业务·总入口)
  03-crossborder-tech-aws.md     模块3 跨境(技术+AWS)
  03b-crossborder-collection-deepdive.md  跨境收款七环节
  03d-trade-settlement.md        跨境贸易模式与结算方式
  03c-crossborder-players/       跨境收款企业画像
支付牌照术语速查.md   跨模块通用牌照速查(根目录)
