...
```mermaid
flowchart TB
    Z["⓪先认识卡<br/>(起源/类型/卡面信息)"] --> A["①卡支付的本质<br/>(推vs拉、信任问题)"]
    A --> B["②四方模型与参与方<br/>(五角色+卡组织)"]
...
## 1.5 先认识银行卡：起源、类型、一张卡上有什么 🔧

> 🔑 既然"从卡讲起"，先花一节认识**卡本身**——它从哪来、有哪几类、一张卡上藏了哪些信息。这是后面四方模型/收单/安全的实物起点。（本节为 🔧 行业公知；起源年份为稳定史实，技术机制指向技术篇，不在此展开。）

### 1.5.1 起源：信用卡怎么来的（简史）🔧

🔧 银行卡解决的原始痛点是"**带现金不方便、商户不认陌生人的赊账**"——用一张卡 + 一个第三方信用，替代现金与熟人赊账。关键节点（行业公知简史）：

| 年代 | 里程碑 | 意义 |
|---|---|---|
| **1950** | **Diners Club（大莱卡）** | 公认第一张多商户通用卡，但发卡方自己兜全责（≈三方模型雏形） |
| **1958** | **BankAmericard**（美国银行，后改名 **Visa**） | 银行发卡的开端，后演化成开放的四方网络 |
| **1966** | **Master Charge**（后改名 **Mastercard**） | 多家银行联营对抗 BankAmericard |
| **2002** | **中国银联（UnionPay）** | 中国的卡组织，统一境内银行卡跨行清算 |

> 💡 演进主线：**单一发卡方全包（三方）→ 众多银行分工的开放网络（四方）**——这正是 §3 四方模型为什么会出现的历史动因。

### 1.5.2 卡的类型：花的是谁的钱 🔧

🔧 卡按"**资金来源**"分三类——这是业务上最本质的分类（技术上 BIN 里有卡种标识，见技术篇 §2.2）：

| 类型 | 花谁的钱 | 资金来源 | 谁担信用风险 | 典型 |
|---|---|---|---|---|
| **借记卡 Debit** | **你自己的钱** | 实时扣你的活期存款账户 | 几乎无（余额不足即拒） | 工资卡、储蓄卡 |
| **信用卡 Credit** | **银行先垫的钱** | 发卡行授信额度，先消费后还款 | **发卡行**（你违约它担） | 各类信用卡 |
| **预付卡 Prepaid** | **预存的钱** | 先充值、再消费（不绑活期账户） | 无（用完即止） | 礼品卡、虚拟卡、部分钱包卡 |

> 🔑 **借记 vs 信用的本质差别**：借记=**拉你自己账户的钱**（呼应 §2.1 拉支付）；信用=**发卡行垫付+授信**——这就是为什么**信用卡有利息/分期/年费而借记卡没有**（§6/§7.1 发卡收入），也是发卡行抢着发信用卡的根因。⚠️ 美国 Durbin 法案对借记卡交换费有特殊管制（见技术篇 §4 路由）。

### 1.5.3 一张卡上有什么：卡面信息与敏感度 🔧

🔧 一张实体卡正反面的字段，决定了支付怎么发起、哪些必须严格保护：

| 字段 | 是什么 | 敏感度 | 用途 |
|---|---|---|---|
| **卡号 PAN**（16 位） | Primary Account Number；**前 6-8 位=BIN** 标识发卡行/卡组织/卡种 | 🔴 高（PCI 须加密/token 化） | 路由+识别账户（技术篇 §2.2 BIN） |
| **持卡人姓名** | 卡主 | 🟡 中 | 核验 |
| **有效期**（MM/YY） | 卡过期日 | 🟡 中 | 校验 |
| **CVV2/CVC2**（背面 3 位） | 卡片验证码 | 🔴 **极高（PCI 红线：绝对禁止存储）** | 证明"卡在手"（CNP 防伪） |
| **磁条 / 芯片（EMV）/ NFC** | 卡数据载体 | 🔴 高 | 受理时读取（机制见技术篇 §3 安全四件套） |
| **PIN（密码）** | 持卡人知识因子 | 🔴 极高（HSM 保护、不落库） | 线下/ATM 鉴权（见 `01c-hsm-payment-cryptography.md`） |

- 🔑 **卡号 ≠ 账户余额所在**：卡号只是"账户的指针 + 路由信息"，钱仍在发卡行账本上（呼应模块0"钱=账本数字"）。
- ⚠️ **安全要求只一句话，细节见技术篇**：卡数据（PAN/CVV/PIN/磁道）是支付最敏感的资产——**防盗刷靠 EMV/Tokenization/3DS、护卡数据靠 PCI-DSS、护密钥靠 HSM**，这"安全四件套+PCI+HSM"是技术篇 `01-cards-tech-aws.md §3` 与 `01c-hsm-payment-cryptography.md` 的主场，本业务篇不展开（§7.1 另讲"卡测试攻击"这一业务侧风控点）。

> 🎯 **交流要点**：能讲"卡按资金来源分借记/信用/预付（花谁的钱）、卡号前 6-8 位是 BIN、CVV/PIN 是 PCI 红线绝不能存"——是聊卡业务的基本功；再点出"卡号只是账户指针、钱在发卡行账本"，就接回了模块0 的地基。
...
```mermaid
flowchart TB
    CH["① 持卡人 Cardholder（你）"] <==>|"交易：刷卡付款 / 交货"| M["② 商户 Merchant（便利店/电商）"]
    CH -.->|"开户 / 持卡"| ISS["③ 发卡行 Issuer<br/>(招商银行) · 服务持卡人侧"]
    M -.->|"签约 / 受理"| ACQ["④ 收单行 Acquirer<br/>(收单机构) · 服务商户侧"]
    ISS <-->|"授权/清算/结算"| CS{{"⑤ 卡组织 Visa/MC/银联（居中协调）"}}
    ACQ <-->|"授权/清算/结算"| CS
```

> 💡 此图呈现四方模型的**对称结构**：**上方** 持卡人 ↔ 商户（对等交易双方）；**下方** 发卡行 / 收单行 分别是两人各自的银行（左右平行、各连其上方的人）；**中间** 卡组织居中连接两家银行。

> 🔑 **怎么读这张图（对称结构）**：
> - **上方两端 = 一笔真实交易的对等双方**：左 **持卡人** ↔ 右 **商户**（你付钱、它交货）。
> - **下方两端 = 各自的银行，左右平行对应**：持卡人的银行是 **发卡行**（左，服务持卡人侧）；商户的银行是 **收单行**（右，服务商户侧）。
> - **正中 = 卡组织**：连接发卡行与收单行、居中协调（路由/清算规则/品牌/仲裁）。
> 一句话：**上面两个人做交易，下面各自的银行替他们走钱，中间卡组织把两家银行接起来。**
...
> 💡 **常见追问：POS 机背后连的是收单行吗？——不能简单这么说，收单是"一条链"不是单一实体。**
>
> ```mermaid
> flowchart LR
>     POS["POS 机<br/>(商户处)"] --> GW["①给商户装机签约的机构<br/>接入/前置系统<br/>(收单行 或 挂靠它的 ISO/PayFac)"]
>     GW --> PROC["②收单处理器 Processor<br/>(授权路由/清算文件)"]
>     PROC --> CS["③卡组织/转接清算<br/>(Visa/MC;中国=银联/网联)"]
>     CS --> ISS["④发卡行<br/>(扣款授权)"]
>     ACQ["收单行 Acquirer<br/>(持牌·担清算最终责任·结钱给商户)"] -.对资金负责.- PROC
> ```
>
> - **物理/系统上**：POS 先连的是**"给商户装机签约的那个机构"的接入系统**——可能是**收单行本身**，也可能是**挂靠收单行的 ISO/PayFac/收单服务商**（这种情况连的是服务商系统，但资金清算责任仍在背后的持牌收单行）。
> - **"收单行"是法律/资金角色**：对这笔交易的**资金清算负最终责任、最后把钱结给商户**——但 POS **不一定直连它本人**。
> - **技术处理由 Processor 做**：报文经接入层后，由处理器送进卡组织 → 路由到发卡行扣款。
> - 🔑 **一句话**：**POS → 收单侧（装机机构+处理器）→ 卡组织 → 发卡行**；"收单行"是收单侧那个**持牌、担责、结钱**的核心，但 POS 连的常是它（或其服务商）的接入系统，而非"直连收单行"这么简单。⚠️ 中国还要分清 POS 走**银联（卡）/网联（涉账户）**转接清算（§4.4 断直连）。
...
#### 4.3.2 PayFac 的核心机理：资金牌照 ≠ 收单资格，自持 acquiring vs 挂靠 sponsor 收单行 📌+⚠️

> 🔑 **理解 PayFac 的钥匙**：要"自己受理买家刷卡（收单）"，光有"能碰钱的牌照"不够，得分两层看——这也解释了"持牌收单机构 vs PayFac"的根本区别（呼应 §4.5 银联商务为何是收单行不是 PayFac）。

| 层 | 是什么 | 谁有 |
|---|---|---|
| **① 资金转移/电子货币牌照**（MTL/MSB/EMI） | 允许你**持有、转移客户资金** | 连连/PingPong/Airwallex/Stripe 都有 |
| **② 卡组织收单资格（Acquiring）** | 允许你**接入 Visa/MC 清算、向发卡行发起授权**——这才是"收单"本身 | 要么自己是卡组织收单会员/持收单牌照，**要么挂靠一家持牌收单行（sponsor）** |

> 🔑 **①≠②**：有 MTL/MSB 能合规碰钱，但**不代表能直连卡组织收单**。**PayFac 的定义性特征，正是"自己没有②、挂靠一家 sponsor 收单行"来实现卡受理**——这就是 PayFac 那层的本质（§4.3）。

**逐家对号——"自持 acquiring" 还是 "挂靠 sponsor"？**

| 公司 | 收单怎么实现 | 可信度 |
|---|---|---|
| **Stripe** | **欧盟/英国**：凭 **EMI 含 acquiring 权限→自己受理**；**美国**：经 SPC 的 MTL + 卡组织通道，**历史上经合作收单行（挂靠）** | 📌 已核查（`02c…/stripe.md §4.2`） |
| **Airwallex** | **自持多国收单牌照**（全栈跨境），在持牌法域更接近自己受理 | 📌 已核查（`03c…/airwallex.md`） |
| **连连 / PingPong** | 欧盟/英国 **EMI 部分理论含 acquiring 可自持**；但作为"跨境 PayFac"，无自持收单资质的法域**很可能挂靠当地 sponsor 收单行** | ⚠️ **逐法域"自持 vs 挂靠"未一手核查**，机制推断 |

> 🔑 **一句话**：**不是"都挂靠"，而是看每家在每个法域有没有自持收单资质（EMI acquiring 权限 / 卡组织收单会员）**——有则自己受理，没有则挂靠 sponsor 收单行（这就是 PayFac 那层）。⚠️ 连连/PingPong 逐法域是自持还是挂靠 sponsor，本轮**未取得一手证据**，写正式材料须核各家收单合作披露/牌照范围。
...
> ⚠️ **常见追问：银联商务算 PayFac 吗？——不算，它是"持牌收单机构(Acquirer)"本身**。PayFac 的定义性特征是"**自己不持收单资质、挂靠一家收单行**"（§4.3.2 第②层）；而银联商务**自持央行收单牌照、是银联收单成员**，是 PayFac 要挂靠的那一层。它也做"聚合中小商户代收"、形态像 PayFac，但那是**它行使自身收单资质的正常业务**，不叫 PayFac。⚠️ **中美术语错位**：PayFac 是美国语境（收单门槛高→催生挂靠层）；中国监管语境只有"**持牌收单 vs 无牌聚合（二清红线，§4.4）**"，没有"PayFac"这个概念——硬套会错位。
...
> ⚠️ **"平台自己收单"是个黑盒——它本身就有多种模式、走不同通道**（上表为简化的卖家视角）。展开看：

| 平台收单模式 | 平台怎么收 | 底层通道 |
|---|---|---|
| **① 自持收单牌照（全栈）** | 平台自己是卡组织收单会员/持牌，亲自受理 | 直连卡组织清算 |
| **② 白标 PayFac/收单引擎** | 不自持，接一个收单服务商当底层引擎 | 经该 PayFac 的 sponsor 收单行（如 Shopify Payments=Stripe Connect 白标，§4.3.1） |
| **③ 多家收单机构路由** | 对接多家收单行/处理器，按 BIN/地区/成本择优 | 多条收单通道并行（冗余+择优，技术篇 §4 网关路由） |
| **④ 聚合多种支付方式** | 除卡外挂 PayPal/Apple Pay/本地钱包/分期 | **每种支付方式走各自轨道**（卡网络/钱包/BNPL，`02b §2`） |

> - 🔑 大平台几乎一定是**多收单通道并行 + 按支付方式分多条轨道**的混合，不是单一通道。买家用 Visa 卡 vs Amazon Pay vs 本地分期，背后走的是**完全不同的清算轨道**。
> - ⚠️ **但这层不影响中国卖家回款的结论**：无论平台底层走哪条收单通道，卖家拿到的都是"**平台 payout 给我的货款**"，卖家只需接回款通道（连连等）——这正是本节"卖家视角"为何能把平台收单简化成一句话。
