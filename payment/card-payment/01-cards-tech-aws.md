...
### 5.1 EMV / 芯片卡 / NFC：技术原理深挖 🔧

> 🔑 业务篇 §1.5.3 指针到此。EMV（Europay/Mastercard/Visa 三家定的全球芯片卡标准）是"卡在场（CP）"防伪的基石。第一性讲清：磁条为什么不安全 → 芯片用什么机制解决 → 联机/脱机怎么授权 → NFC 非接触与手机钱包怎么叠上去。（🔧 成熟行业公知，机制稳定，不逐条引一手。）

#### 5.1.1 痛点：磁条卡为什么能被复制 🔧

🔧 **磁条（magstripe）存的是"静态数据"**——卡号、有效期、持卡人名等**每次刷都一样、明文可读**。于是：
- 用一个几十块的读卡器（**侧录 skimming**）就能把磁道数据复制下来 → 写到一张空白卡上 → **克隆卡**，刷卡机无法分辨真伪（数据完全一致）。
- 根因：**静态数据 = 复制即可重放（replay）**。这是磁条时代盗刷泛滥的技术根源。

#### 5.1.2 EMV 芯片怎么解决：每笔生成"动态密文" 🔧

🔑 **EMV 的核心机制：芯片里有一个安全微处理器 + 一把永不出芯片的密钥，每笔交易现场生成一个一次性的动态密文（cryptogram），无法复制重放。**

```mermaid
sequenceDiagram
    participant C as 芯片卡(安全芯片+密钥)
    participant T as 终端(POS/ATM)
    participant ISS as 发卡行
    T->>C: ①供电+发起,送交易数据(金额/币种/随机数等)
    C->>C: ②芯片内用卡密钥+交易数据+计数器(ATC)<br/>算出动态密文 ARQC(每笔不同)
    C->>T: ③返回 ARQC
    T->>ISS: ④随ISO8583授权请求上送 ARQC
    ISS->>ISS: ⑤发卡行用对应密钥重算,比对 ARQC 是否吻合
    ISS->>T: ⑥验证通过→授权(可回送 ARPC 让卡再验发卡行)
```

- **ARQC（Authorization Request Cryptogram，授权请求密文）**：芯片用**卡内密钥 + 本次交易数据（金额/币种/终端随机数）+ 交易计数器 ATC** 算出的 MAC/密文。因为每笔的交易数据和 ATC 都不同，**ARQC 每笔都不一样**——录下这一笔也没法用到下一笔（防重放）。
- **密钥永不出芯片**：算密文在芯片内部完成，攻击者拿不到密钥，也就**无法伪造合法 ARQC**（呼应 §6 / `01c` HSM——发卡行侧的对应密钥由 HSM 保护）。
- **ATC（计数器）**：芯片内单调递增的交易序号，防"同一密文被重复使用"。
- 🔑 **一句话**：磁条是"静态数据、复制即用"；EMV 是"**动态密文、一次一密**"——这就是芯片为什么"无法被克隆"的根本。

#### 5.1.3 联机 vs 脱机授权：芯片还能自己决策 🔧

🔧 EMV 不只是生成密文，芯片还能参与"这笔该不该批"的决策（这是磁条做不到的）：
- **联机授权（Online）**：芯片生成 ARQC → 上送发卡行实时验证+决策（主流，金额较大/有网络时）。
- **脱机授权（Offline）**：无网络/小额场景，芯片用内置规则（脱机数据认证 SDA/DDA/CDA + 风险参数）**自己批准或拒绝**，生成 TC（成交密文）或 AAC（拒绝密文）——如部分地铁闸机、飞机上刷卡。
- **CVM（持卡人验证方式）**：芯片按优先级决定怎么验人——**输 PIN（线下主流）/ 签名 / 免验证（小额免密）/ 联机 PIN**。这套规则也存在芯片里。

#### 5.1.4 接触式 vs NFC 非接触：同一套芯片逻辑，换了通信方式 🔧

| | 接触式（插卡） | 非接触 NFC（闪付/碰一碰） |
|---|---|---|
| 通信 | 金属触点供电+通信 | 射频感应（13.56MHz），近距离无线 |
| 密文机制 | **同样生成动态密文** | **同样生成动态密文**（EMV Contactless 标准） |
| 体验 | 插入停留 | 靠近即走（小额常免 PIN） |

- 🔑 **NFC/闪付 = EMV 芯片的"非接触形态"**——底层还是"每笔动态密文"那套 EMV 逻辑，只是把"插触点"换成"射频感应"。安全性同源于动态密文，不是磁条。

#### 5.1.5 手机钱包（Apple/Google Pay）：手机是一张"被令牌化的虚拟 EMV 卡" 🔧

> ⚠️ **先破一个常见误解**：手机钱包**不读取、也不需要你实体卡的物理芯片**——它跟"你那张卡是芯片卡还是磁条卡"无关。手机是用 **DPAN + 手机自身的安全芯片**，**模拟一张虚拟 EMV 卡**，复用的是 EMV"动态密文"这套**机制/标准**，不是你实体卡的芯片。

🔑 Apple Pay/Google Pay **不是把你的真卡号存手机里发出去**，机制分两步：

**① 绑卡时（provisioning）——DPAN 是新申请的，不来自卡芯片**
- 你加卡时，手机把真卡号 PAN 交给**卡组织令牌服务（Visa VTS / Mastercard MDES）**→ 卡组织联合**发卡行**做身份验证（ID&V）后**新生成一个 DPAN（Device PAN，设备令牌）** 下发到手机。
- 🔑 **DPAN 是"凭卡号在线新发"的替身号，不读取你实体卡的芯片**——所以哪怕你绑的是张**纯磁条老卡**，只要发卡行/卡组织支持令牌化，一样能加进 Apple Pay。**前提是发卡行+卡组织支持，不是卡芯片支持。**

**② 付款时——动态密文由"手机的安全芯片"生成**
- 手机里存的是 **DPAN**（非真卡号），用**指纹/面容**做持卡人验证（CVM）。
- 付款时手机经 NFC 发出 **DPAN + 该笔动态密文（cryptogram）**——这密文是**手机自身的安全单元（Secure Element / 可信执行环境）**用 DPAN 对应的令牌密钥**现场生成**的；**你实体卡的物理芯片全程不参与**。
- **真卡号 PAN 从不出现在手机、也不发给商户**。

> 🔑 **一句话**：**手机= 一张"被令牌化的虚拟 EMV 卡"**——DPAN 来自绑卡时的在线令牌化、动态密文由手机安全芯片生成；它**复用 EMV 机制和 NFC 信道，但与你实体卡的芯片无关**。安全性 = **EMV 动态密文（防重放）+ Tokenization（真卡号不暴露 DPAN）** 双层叠加（呼应 §5 Tokenization、`02b §2.4` 钱包"token 化卡"底座）。

#### 5.1.6 责任转移（Liability Shift）：EMV 普及的商业推手 🔧

🔧 EMV 不只是技术，还有**商业杠杆**：卡组织设定的"**责任转移**"规则——若一笔欺诈交易，**支持 EMV 的一方不担责、不支持的一方担责**。例如美国 2015 年 EMV liability shift 后，若商户没上 EMV 终端、收了克隆卡，**欺诈损失由商户承担**（而非发卡行）。这逼着全球商户换芯片终端——和 3DS 的责任转移是同一套"用赔付规则驱动技术采用"的逻辑。

> 🎯 **交流要点**：能讲"磁条=静态数据可复制、EMV=芯片内密钥+ARQC 动态密文一次一密、ATC 防重放、联机/脱机授权、NFC 是 EMV 非接触形态、手机钱包=EMV+Tokenization 双层、liability shift 是普及推手"——直击卡在场防伪的工程实底，远超"芯片更安全"的表层说法。
