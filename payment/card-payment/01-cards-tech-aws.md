...
#### 5.1.5 手机钱包（Apple/Google Pay）：在 NFC 之上再叠 tokenization 🔧

🔑 Apple Pay/Google Pay **不是把你的真卡号存手机里发出去**，而是在 EMV 非接触之上**再叠一层令牌化（§5 Tokenization）**：
- 手机里存的是**设备令牌 DPAN（Device PAN）**，不是真卡号 PAN；指纹/面容做持卡人验证（CVM）。
- 付款时手机经 NFC 发出的，是 **DPAN + 该笔的动态密文/cryptogram**——**真卡号 PAN 从不出现在手机、也不发给商户**。
- 🔑 所以手机钱包安全性 = **EMV 动态密文（防重放）+ Tokenization（真卡号不暴露）** 双层叠加（呼应 §5 Tokenization、`02b §2.4` 钱包"token 化卡"底座）。
