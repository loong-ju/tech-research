...
### 1.1 端到端架构骨架：一笔跨境交易要穿过哪些技术层 🔧

> 🔑 上面是"四块挑战"的**领域视角**；下面是**一笔交易从进来到落地**的**端到端骨架**——把全篇各章节挂在这条主线上，先有全局，再下钻。**合规/风控不是某一段，而是贯穿始终的横切层**（详见 §4）。

```mermaid
flowchart TB
    subgraph IN["① 接入/受理层"]
        API["API/收银台/报文网关<br/>(卡授权·SWIFT报文·本地支付方式)"]
    end
    subgraph PROC["② 处理/编排层"]
        ORCH["交易编排+状态机<br/>(幂等·重试·路由)"]
    end
    subgraph RISK["③ 风控/合规横切层(贯穿全程)"]
        KYC["准入KYC/KYB"]
        KYT["交易监控KYT"]
        SANC["制裁筛查"]
    end
    subgraph CORE["④ 账务/清结算层"]
        LEDGER["多币种账本"]
        FX["汇率引擎/外汇头寸"]
        CLR["清算轧差(卡组织/SWIFT/本地)"]
    end
    subgraph BACK["⑤ 对账/处置层"]
        RECON["多方多币种对账"]
        CASE["告警处置/SAR上报"]
    end
    API --> ORCH --> CORE
...
| 层 | 干什么 | 本篇章节 |
|---|---|---|
| **① 接入/受理** | 卡授权(ISO8583)、SWIFT 报文、本地支付方式(LPM)接入 | §2（报文）、§2.5（卡组织跨境·待补）|
| **② 处理/编排** | 交易状态机、**幂等/重试**、智能路由 | §5.x NFR（跨境落地·待补）|
| **③ 风控/合规(横切)** | 准入 KYC/KYB → 交易监控 KYT → 制裁筛查 → 持续监控 | **§4（全生命周期主线）** |
| **④ 账务/清结算** | 多币种账本、汇率/头寸、清算轧差 | §3（账务）、§2.5（卡组织清算）|
| **⑤ 对账/处置** | 多方对账、告警 case→SAR 上报 | §5（对账）、§4.6（处置上报）|

> 🎯 **架构师视角**：跟支付公司聊技术，能先甩出这张"**接入→编排→(横切风控)→清结算→对账**"骨架、再按需下钻到任一层，比上来就讲某个点更显体系。**风控合规是"横切层"不是"某一步"**——这是支付架构区别于普通交易系统的根本特征。
...
## 4. 跨境合规与风控：贯穿交易全生命周期的横切层（最硬的跨境技术）

📌 **第一性**：跨境最大的合规风险是**违反制裁**（罚款可达数十亿美元甚至刑责）+ **被用于洗钱/欺诈**。但合规**不是交易链路里的某一步，而是从客户进门到退出、贯穿全生命周期的横切层**——只讲"制裁筛查"这一个深点，是只见树木不见森林。本节按**生命周期主线**组织。

### 4.0 合规风控生命周期总览：从准入到退出 🔧

> 🔑 把分散的 KYC/KYB/KYT/制裁筛查/AML/上报**串成一条主线**——一个客户从入驻到交易到持续监控到处置，合规系统在每一环各做什么。**这是体系，不是孤立工具的堆叠。**

```mermaid
flowchart TB
    A["① 准入 Onboarding<br/>KYC(个人)/KYB(企业)<br/>身份核验·UBO穿透·风险分级·入驻制裁筛查"] --> B["② 交易监控 KYT<br/>每笔实时:规则+模型打分<br/>制裁筛查(收付款人+银行)·限额·异常模式"]
    B --> C["③ 持续监控 Ongoing<br/>周期复审·名单更新回溯重筛(rescreening)<br/>行为画像漂移·触发式EDD"]
    C --> D["④ 处置与上报 Case&Report<br/>告警→case management→分析师复核<br/>真命中→冻结/退出+SAR/STR上报监管"]
    C -.正常客户持续交易.-> B
    D -.解除/误报.-> B
```

| 生命周期阶段 | 核心动作 | 本篇小节 | 横向专题 |
|---|---|---|---|
| **① 准入 Onboarding** | KYC/KYB、UBO 穿透、风险分级、入驻筛查 | **§4.1** | `_topics/62-compliance §2` |
| **② 交易监控 KYT** | 每笔实时打分、制裁筛查、限额、异常模式 | **§4.2** + **§4.3**(制裁) | `62 §3.2.1` KYT |
| **③ 持续监控 Ongoing** | 周期复审、rescreening、画像漂移、EDD | **§4.5** | `61` 风控分层 |
| **④ 处置与上报** | case management、复核、冻结、SAR/STR | **§4.6** | `61 §5`、`62 §3.2` |
| **(横切)反洗钱 AML + 外汇申报** | 模式监测、SAFE 申报 | **§4.4** | `62 §3` |

> 🎯 **架构师视角**：聊合规别只会"制裁筛查模糊匹配"——能讲"**准入(KYC/KYB)→交易监控(KYT)→持续监控(rescreening/复审)→处置上报(case→SAR)** 的全生命周期闭环，且每环 AWS 怎么落地"，才是体系化认知。制裁筛查是其中**横切多个阶段**的一个能力（入驻筛、每笔筛、回溯筛），不是独立的一步。

### 4.1 准入：KYC / KYB 怎么做（Onboarding）🔧

> 🔑 **第一性**：钱要能流动，先得知道"对面是谁、是不是真实合法主体、背后实际控制人是谁"——这是**反匿名**，一切合规的地基（通用原理见 `_topics/62-compliance §2`，这里讲**跨境特有的工程做法**）。

**① KYC（个人）vs KYB（企业）采什么、验什么**：

| | KYC（Know Your Customer·个人） | KYB（Know Your Business·企业，跨境收款/收单主战场）|
|---|---|---|
| **采集** | 姓名、证件、地址、出生日期、人脸 | 营业执照/注册证、注册地、股权结构、董事、**UBO 名单**、经营范围、店铺/网站真实性 |
| **核验手段** | 证件 OCR + **活体检测/人脸比对**、地址证明、政府/征信库核验 | **企业注册库核验**(各国工商/公司注册处)、制裁&PEP&不良媒体筛查、银行账户验证、**网站/店铺抓取核验经营真实性** |
| **跨境难点** | 多国证件格式、多语言姓名转写 | **穿透境外多层股权找 UBO**(壳套壳)、各国注册库数据源不一、离岸架构 |

**② 核心动作：UBO 穿透（跨境 KYB 的命门）** 🔴

```mermaid
flowchart TB
    M["拟入驻商户<br/>(干净壳公司)"] --> L1["第一层股东<br/>(另一家公司)"]
    L1 --> L2["第二层股东<br/>(离岸实体)"]
    L2 --> UBO["最终受益人 UBO<br/>(自然人·通常持股≥25%)"]
    UBO -.每个UBO都要.-> SCR["制裁/PEP/不良媒体筛查"]
    M -.直接对手方也筛.-> SCR
```

- 制裁规避最典型手法 = **"干净壳 + 脏 UBO"**：门面公司本身不在名单，背后实控人在。所以**必须层层穿透股权、把每个 UBO（通常持股≥25%）拉出来单独筛制裁/PEP**——只筛直接对手方会漏（呼应 §4.3 ③）。
- 跨境难在：境外多层、离岸地不公开股东、需买商业数据(企业图谱)补全。

**③ 风险分级（risk-based onboarding）**：把"国家风险(高风险法域)+行业风险(博彩/加密/外贸)+股权复杂度+制裁/PEP 命中"综合成入驻风险分 → **低风险简化尽调(SDD)、高风险强化尽调(EDD)**。这个分也驱动后续交易监控的阈值松紧。

- ☁️ **AWS**：证件 OCR/人脸比对用 **Rekognition + Textract**；多语言姓名处理 **Comprehend**；企业图谱/UBO 关联用 **Neptune（图）**；入驻筛查复用 §4.3 的 OpenSearch 引擎；KYB 综合评分可用 **Bedrock Agent**（读注册库+股权+命中→出带理由的风险分，呼应 reference 的 KYB 评分 Agent，human-in-the-loop）；客户档案/决策留痕入审计库（S3 + 不可变 Object Lock）。

### 4.2 交易监控：KYT 怎么做（Know Your Transaction）🔧

> 🔑 准入只解决"进门时干净"，但**交易行为会变**——KYT 是**每笔交易实时**问"这笔正常吗"（通用实现见 `62 §3.2.1`，这里讲跨境编排）。

**① 一笔交易实时要并行跑哪些检查**：

```mermaid
flowchart TB
    TX["一笔跨境交易进来"] --> P1["制裁筛查<br/>(收款人+付款人+中间行·§4.3)"]
    TX --> P2["规则引擎<br/>(限额·高风险国·频率·拆分)"]
    TX --> P3["风控模型打分<br/>(欺诈/异常·§61)"]
    P1 & P2 & P3 --> AGG{"综合决策<br/>(任一硬命中即拦)"}
    AGG -->|放行| OK["进清结算(§3)"]
    AGG -->|挂起| HOLD["拦截→人工/Agent复核(§4.6)"]
```

- **三类并行检查**：① **制裁筛查**(硬命中即拦，§4.3)；② **规则引擎**(单笔/日累计限额、高风险国家、短时高频、**结构化拆分**即化整为零规避申报阈值)；③ **风控模型打分**(欺诈/异常行为，三代演进见 `61 §3`)。
- 🔑 **跨境特有的监控点**：**化整为零拆分**(规避 SAFE/$1万 申报线)、**同一受益人多账户归集**、**贸易背景与资金流不匹配**(无真实贸易的"空转"对敲洗钱)、**高风险走廊**(资金流向高风险法域)。
- **必须低延迟**：交易链路同步筛查，否则卡住支付——所以制裁筛查引擎要毫秒级（§4.3 的 OpenSearch + 候选召回设计正为此）。

- ☁️ **AWS**：实时规则引擎用 **Lambda/ECS + ElastiCache**(限额计数器/滑窗)；模型打分 **SageMaker/Fraud Detector**；关联团伙/对敲用 **Neptune 图**(呼应 reference 的 GNN)；流式监控 **Kinesis + Flink**；决策编排 **Step Functions**。

### 4.3 制裁名单筛查（Sanctions Screening·横切①②③阶段）
...
> 🔑 **准确表述**：筛查引擎 = **"模糊匹配/相似度搜索引擎"为内核(②③④) + "规则/策略引擎"为外壳(⑤ + ①清洗规则)** 的复合体。**内核技术上最像一个专用搜索引擎 + 记录链接系统，不是规则引擎**；规则决定"像了之后怎么处置"，不决定"像不像"。这正是为什么 §4.3 末尾 AWS 用 **OpenSearch**(自带模糊匹配/编辑距离/语音分析器/n-gram)做匹配引擎、而非规则引擎产品。
>
> 🔧 **演进**：传统是 ②③④ 全确定性算法(可解释、可审计=合规刚需，但误报率 90%+)；进阶在 ④ 用 ML 压误报；前沿(§4.3 ⑥)是**不改内核、在 ⑤ 之后的复核环节用 LLM Agent 读全上下文消歧**。⚠️ **红线**：内核打分必须**可解释**(为什么命中、相似度怎么算)，纯黑盒模型在制裁这种零容忍领域过不了审计——所以确定性匹配引擎至今是骨干，AI 是增强非替代。
...
> 💡 **追问：用当前领先的 LLM（如 Claude Opus 4.8 这代）是不是有明显优势？——有，但红线不变。** 🔧 行业级判断（不含跑分、不断言某模型最优）：
>
> **优势明显，且恰好命中合规四大硬要求：**
>
> | 合规提效的硬要求 | 领先 LLM 的能力优势 |
> |---|---|
> | 告警分诊给"带理由+置信分"的处置 | **更强的多步推理**（如 adaptive thinking——模型自定推理深度），理由经得起审计 |
> | 读"全部上下文"做消歧（交易+档案+名单别名/DOB/国籍/关联） | **超长上下文**（当前领先模型已达 1M token 级）——一次塞下海量名单+交易史+SOP，不必切片丢信息 |
> | 可审计、可解释、决策留痕（合规红线） | **思维过程可总结**（summarized reasoning）——给人可复核的推理摘要 |
> | 起草 SAR/STR、复核纪要、跨语言实体对齐 | 更准、更结构化的长文生成 + **更可靠的多步 Agent 编排**（读多源→查关联→生成→回写审计） |
>
> - 🔑 合规提效要的是"**长上下文 + 深推理 + 可解释 + 准确结构化输出 + 可靠 Agent 编排**"——这几样正是当前领先 LLM 相对早期模型的代差所在。**模型越强，能在"有人兜底的复核环节"自动化掉的工作量越大、误报清理越准、上报草稿质量越高。**
> - ⚠️ **但反直觉的关键点：模型变强放宽的是"复核提效(治标)"的天花板，不是"源头砍告警(治本)"的红线**。最终拦截/放行/上报的决策**必须由持牌合规人复核担责**（human-in-the-loop 是监管红线、不是技术选择，见上方红线条）；初筛环节受可解释性要求约束。所以**不是模型不够强，而是治本的空间被合规红线压窄了**（呼应 ⑤）。
> - 🔑 **一句话**：领先 LLM 让"**AI 给建议+理由+证据、人拍板**"这个模式做得更快更准更全（优势巨大、立竿见影），但"**人拍板**"这一环不会因为模型更强就被取消。能力提升变现在**分诊/消歧/起草/实体对齐/Agent 编排的质量与自动化比例**上，而非"让 AI 直接做拦截决策"。
> - ⚠️ 可信度：上表能力维度（长上下文/深推理/可解释/Agent 编排）为🔧行业级判断；Claude Opus 4.8 的 1M 上下文、adaptive thinking、summarized reasoning 经 claude-api 一手校准，作"当前领先模型具备这些特性"的例证，非"某模型一定最优"的断言。
...
🔧
- **AML**：监测可疑交易**模式**（拆分、异常频率、高风险国家、对敲空转），积累证据，上报可疑活动报告（SAR/STR，见 §4.6）。与 KYT(§4.2) 的关系：KYT 是"每笔实时判"，AML 是"跨笔跨时间看模式"——两者互补。
- **外汇申报（跨境特有）**：中国向 **SAFE（外管局）** 申报跨境收支——**交易真实性核验**(有无真实贸易背景)、限额管理、国际收支申报(BOP)。这是境内支付没有、跨境独有的合规层。
- ☁️ **AWS**：AML 模式监测用 SageMaker/Fraud Detector + 图分析（Neptune，识别关联交易团伙/对敲，呼应 reference PPT 的 GNN）；申报数据管道用 Glue/Athena。

### 4.5 持续监控：名单回溯重筛 + 周期复审（Ongoing）🔧

> 🔑 **最易被忽略、却是合规硬动作**：客户入驻时干净 ≠ 永远干净。合规要**持续盯**——这是把"一次性检查"升级为"生命周期监控"的关键一环。

- **① 名单更新后回溯重筛（rescreening）** 🔴：制裁名单不定期更新(OFAC 一天可能多次)。名单一变，**不只筛新交易，还要把全部存量客户/在途交易回溯重筛一遍**(昨天合规的今天可能上榜)。这是制裁硬要求，也是**算力大头**(见 §4.3 ②)。
- **② 周期性复审（periodic review）**：按风险等级定期重做尽调——高风险客户(如每年)、低风险(如每3年)重新核验 KYB 资料、UBO 变更、经营状况。
- **③ 行为画像漂移触发 EDD**：客户交易模式突变(突然大额、新增高风险走廊)→触发**强化尽调(EDD)**补充材料。
- ☁️ **AWS**：回溯重筛用批处理 **Batch/EMR**(对存量库全量跑 OpenSearch 匹配)；复审到期调度 **EventBridge + Step Functions**；画像漂移检测 **SageMaker**(基线偏离告警)。

### 4.6 处置与上报：从告警到 SAR（Case Management）🔧

> 🔑 前面各环节产生的**告警/命中**最终要有人处置、并按法域上报监管——这是生命周期的闭环出口。

```mermaid
flowchart TB
    ALERT["告警来源:制裁命中/KYT异常/AML模式"] --> CASE["Case Management<br/>(汇总同一主体的所有告警+证据)"]
    CASE --> REV["合规分析师复核<br/>(读全上下文判定)"]
    REV -->|"误报 false positive"| WL["放行+加白名单+留痕"]
    REV -->|"真命中/可疑"| ACT["处置:冻结/拒付/退出客户"]
    ACT --> RPT["按法域上报监管<br/>(美SAR·多地STR·OFAC blocking report)"]
    RPT -.跨多法域可能多份.-> RPT2["跨辖区双报(如中拉/中新)"]
```

- **Case Management**：把同一主体的多个告警(制裁+KYT+AML)**聚合成一个案件**+证据链，而非孤立处理——这是合规运营的核心系统。
- **三档处置(非二元)**：误报→放行+白名单留痕；真命中→冻结/拒付/退出；疑似证据不足→转 EDD(§4.5)再决策。
- **上报(跨境特有的多辖区)**：美国 **SAR**、多数地区 **STR**、OFAC 命中要 **blocking report**(冻结后10工作日内)；**一笔交易触及多法域可能要向多个监管各报一份**(见 `62 §3.2`)。
- 🔑 **AI 提效在这一环最显著**(详见 §4.3 ⑥)：Agent 做告警分诊/消歧/起草 SAR，**但拍板必须持牌合规人**(human-in-the-loop 红线)。
- ☁️ **AWS**：Case Management 用 **DynamoDB/Aurora**(案件状态机)；证据/决策**不可变留痕** S3 + Object Lock(审计合规)；SAR 草稿生成 **Bedrock Agent**；全流程审计日志 CloudTrail。

> 📖 KYC/KYB/KYT/AML/制裁的**通用原理**见 `_topics/62-compliance`、风控分层/反欺诈三代见 `_topics/61-risk-fraud`；本节聚焦**跨境特有工程做法 + 生命周期串联**。reference PPT 里有 KYB 评分 / AML 起草 SAR 的 Agent AWS 案例。
...
flowchart TB
    ...
    subgraph COMPLY["跨境合规/风控(横切·全生命周期)"]
        KYC["准入KYC/KYB Rekognition+Neptune图(UBO)"]
        KYT["交易监控KYT Lambda+SageMaker"]
        SCREEN["制裁筛查 OpenSearch模糊匹配"]
        AML["AML+回溯重筛 SageMaker+Neptune+Batch"]
        CASE["处置上报 Case+SAR Bedrock"]
    end
    subgraph RECON["跨境对账"]
...
| **准入 KYC/KYB(§4.1)** | Rekognition/Textract(证件人脸) + Neptune(UBO图) + Bedrock(评分Agent) |
| **交易监控 KYT(§4.2)** | Lambda/ECS + ElastiCache(限额) + SageMaker(打分) + Neptune(对敲) |
| 制裁名单筛查(§4.3) | **OpenSearch（模糊匹配）** + Comprehend |
| AML/反洗钱(§4.4) | SageMaker/Fraud Detector + Neptune（图） |
| **持续监控/回溯重筛(§4.5)** | Batch/EMR(全量重筛) + EventBridge/Step Functions(复审调度) |
| **处置上报 Case/SAR(§4.6)** | DynamoDB(案件) + S3 Object Lock(不可变留痕) + Bedrock(SAR草稿) + CloudTrail |
...
5. **合规是横切全生命周期的层，不是一步**：准入(KYC/KYB·UBO穿透)→交易监控(KYT)→制裁筛查(横切三阶段·模糊匹配是最硬难点·误报率核心矛盾→OpenSearch)→持续监控(回溯重筛rescreening+周期复审)→处置上报(case→SAR)。
