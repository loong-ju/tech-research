...
| `01c-hsm-payment-cryptography.md` | 📝 模块1·深化 | CloudHSM与Payment Cryptography实战(经AWS官方文档核实)：HSM=带密钥的运算器、支付密钥层级(LMK→KEK→PEK/PVK/BDK/CVK/EMV-MK、TR-31/34/DUKPT)、三服务选型(Payment Cryptography支付专用/CloudHSM通用/KMS数据)、两API平面(Control Plane密钥管理+Data Plane运算)、**收单PIN翻译(TranslatePinData)**、**发卡PIN验证/CVV/EMV(VerifyPinData/CardValidationData/VerifyAuthRequestCryptogram)**真实调用链、密钥全生命周期 |
