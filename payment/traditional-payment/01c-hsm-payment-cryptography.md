...
> 📌 收单的核心密钥交换 = **"终端→收单→卡组织→发卡行"这条链上的每一跳**。PIN 在每跳用不同密钥加密，靠的就是相邻两方预先交换好的密钥（这正是 §5 PIN 翻译的前提）。

**全支付域的密钥交换场景**：

| 业务域 | 谁和谁 | 典型密钥 | 业务场景 |
|---|---|---|---|
| 收单-终端 | 收单机构 ↔ POS/ATM | BDK/IPEK | 新终端上线注入密钥 |
| 收单-网络 | 收单机构 ↔ 卡组织/网联 | ZMK→ZPK、MAC | 接入卡组织/转接清算 |
| 发卡-网络 | 发卡行 ↔ 卡组织/转接 | ZMK→ZPK | 发卡行接入清算 |
| 发卡-制卡 | 发卡行 ↔ 卡个人化局/制卡商 | 个人化密钥、EMV 密钥 | 批量制卡个人化 |
| 银行间/代理行 | 银行A ↔ 银行B（代理行） | BIK、PEK、MAC | 跨行/跨境代理行（模块3） |
| 第三方支付-通道 | 支付机构 ↔ 合作银行/通道 | 渠道加密/签名密钥 | 第三方支付接银行（模块2） |
| 商户-网关 | 商户 ↔ 支付网关 | API密钥、报文签名密钥 | 商户接入收单（模块2） |
| 机构-云 | 本地 HSM ↔ AWS Payment Cryptography | TR-34建信→TR-31 | 密钥迁移上云（§2.4） |
...
>
> 📌 **所以准确说**：分量合成在 **AWS 之外的客户侧**——但生产环境应是"**客户自己的物理 HSM 内合成**"（明文分量/明文密钥绝不落普通服务器），**绝非** sample 演示的"普通主机内存里 XOR"（那个仅供测试）。无论哪种，合成后都用 TR-34 导入 AWS。详见 §2.4。
...
#### 2.3.1 密钥怎么进终端：BDK 注入 与 SoftPOS
