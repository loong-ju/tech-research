...
|---|---|---|---|---|
| 收单-终端 | **收单机构**（持 BDK） | → POS/ATM 终端 | 派生的 **IPEK**（BDK 不给，留 HSM） | 终端上线时 KIF 物理注入或 RKL 远程加载（§2.3.1） |
...
| 机构-云 | **机构本地 HSM**（KDH） | → **AWS Payment Cryptography**（KRD） | KEK/ZMK（TR-34 建信）→ 工作密钥（TR-31） | 密钥迁移上云（§2.4） |
