...
- **第三方收款服务商**：本质是"跨境 PayFac + 换汇"（模块1深化 01b 已讲透）。
...
📌 **第三方跨境收款服务商**（出海卖家核心工具）：连连/PingPong/Airwallex/Payoneer/万里汇——本质是**跨境 PayFac + 多币种账户 + 换汇 + 多国牌照**（模块1深化 01b 已详解）。
- 合规底座：SAFE《支付机构外汇业务管理办法》（汇发〔2019〕13号，已核查）。

📖 详见报告第九部分 + 模块1深化 `01b`（跨境收款公司=跨境PayFac 的拆解）。
...
- **稳定币如何重做共同账本** → 模块4
- **跨境收款公司=跨境PayFac 的产业链拆解** → 模块1深化 `01b`
...
A：不是银行，是持牌支付机构（境内持 SAFE 跨境外汇业务资质，境外持各国 MSB/EMI/MSO 等牌照）。它们靠"境外本地账户收外币 + 境内结汇付人民币 + 向外管局申报"完成合规收款。详见模块1深化 01b 和报告第九部分。
