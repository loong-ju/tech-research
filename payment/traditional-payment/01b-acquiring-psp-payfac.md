...
flowchart TB
    subgraph 收单系统["收单系统(逻辑功能集合)"]
        ...
        SETTLE["清结算/对账系统<br/>(仅指收单机构自营的'对商户结算'段)"]
        MM["商户管理<br/>入网/KYB/费率"]
        RISK["风控<br/>反欺诈/拒付"]
    end
```

> ⚠️ **注意**：此处"清结算/对账"指**收单机构自己运营的那一段**——即"收到清算资金后扣费分账、把钱结给商户 + 商户对账"。**整个产业链的清结算是分层的、分属不同主体**（网络清算归卡组织、最终资金结算归央行/银联网联），详见下面 1.6.5。
...
## 第一性追问 1.6.5：清结算属于谁？——它是分层链路，不是单一主体

很多人问"清结算系统属于收单机构还是卡组织"。答案是：**都不是单一归属——清结算是一条贯穿多个主体的分层链路，每个主体运营自己那一段。**

📌 **第一性**：清结算发生在"**每一层账本的边界**"。回到模块0公理——支付是改账本，清结算就是"算清账本之间该怎么改、然后真的改"。卡链路有好几层账本边界，所以清结算分层、分段。

```mermaid
flowchart TB
    CS["①网络清算<br/>卡组织(Visa/MC/银联)<br/>轧差算各成员行净额,下发清算文件<br/>★只算账不划钱(Clearing)"]
    CB["②最终资金结算<br/>央行/清算机构(Fedwire / 中国:银联·网联+央行大额系统)<br/>成员行间真划央行货币<br/>★真划钱,finality(Settlement)"]
    ACQ["③对商户结算<br/>收单机构<br/>收到清算资金后扣费/分账,结给商户"]
    ISS["④对持卡人记账<br/>发卡行<br/>持卡人账户记账、出账单"]
    CS --> CB --> ACQ
    CS --> ISS
```
...
| 层级 | 谁运营这段 | 干什么 | 账本边界 |
|---|---|---|---|
| **① 网络清算** | **卡组织** | 轧差算"发卡行A该付收单行B多少净额" | 发卡行↔收单行 |
| **② 最终资金结算** | **央行/清算机构** | 成员行间真划央行货币,finality | 成员行↔成员行(央行账本) |
| **③ 对商户结算** | **收单机构** | 扣手续费/分账,把钱结给商户 | 收单机构↔商户 |
| **④ 对持卡人记账** | **发卡行** | 持卡人账户记账、出账单 | 发卡行↔持卡人 |

⚠️ **关键澄清**：**卡组织通常只做"清算"(算净额)，不直接做大额资金的"最终结算"——最终结算落在央行/清算银行账本上**(卡组织有自己的结算银行帮它完成成员行间资金划拨)。这正是模块0"清算≠结算"在卡体系的体现。

### 中国特殊性（断直连后,网络清算归国家队）
| 清算类型 | 中国由谁做 |
|---|---|
| 银行卡网络清算 | **中国银联** |
| 第三方支付(微信/支付宝)清算 | **网联**(断直连后强制) |
| 最终资金结算 | 央行**大额支付系统/超级网银** |

> 🎯 **交流要点**：聊"清结算"先问"**你说的是哪一层——网络清算、最终结算、还是对商户结算？**"。能区分这三层,说明你理解清结算分布在产业链上、而非一个集中系统——这和"收单系统功能层≠主体边界"是同一种思维。收单机构能自营的只是"③对商户结算"这一段;网络清算这层在中国被银联/网联国家队垄断,机构不得绕过(否则二清)。

### 清算的"多层复现"：网络清算 vs 内部清分

⚠️ 进一步澄清一个易混点：**"清算"这个词在链路上会反复出现，但每层含义不同。** 第一性看——模块0说清算的本质是"**算清谁该得多少**"，**任何主体只要收到一笔钱、需要算清这笔钱怎么分配，它内部就有一套"清算"逻辑**。为区别于卡组织的"网络清算"，主体内部的通常叫**清分(split)**。

```mermaid
flowchart TB
    L1["卡组织:网络清算<br/>成员行间轧差净额(对外/跨主体)"] --> L2["收单机构:内部清分<br/>总额→各商户,扣MDR(对内)"]
    L2 --> L3["平台型商户:平台分账<br/>货款→平台抽佣+各子商户/服务方(对内)"]
    L3 --> L4["子商户/个人:收到最终归属的钱"]
```

| 主体 | 有"清算"吗 | 它的"清算"叫什么/做什么 |
|---|---|---|
| **卡组织** | ✅ | **网络清算**:成员行间轧差(跨主体) |
| **收单机构** | ✅ | **内部清分+结算payout**:把收到的总额按商户拆开、扣MDR、按T+N打款(1.6.5的③) |
| **普通单一商户**(便利店) | ❌基本没有 | 收到的结算款就是自己的,无需再拆 |
| **平台型商户**(美团/淘宝/Uber) | ✅且很重 | **平台分账/清分**:把货款拆给平台抽佣+众多卖家/骑手/司机 |

💡 **案例**:你在美团付50元,美团收到后内部清分——商家42、骑手6、平台抽佣2。这套"一笔钱拆给多方"就是平台型商户的内部清算,本质即模块1深化的 **PayFac/分账**(主商户收总额、清分给子商户)。

> 📌 **一句话**:网络清算(卡组织,对外轧差) 和 内部清分(收单/平台,对内拆账) 都叫"清算",本质都是模块0那句"算清谁该得多少",只是发生在不同主体、不同方向。所以**收单和平台型商户都有自己的"清算",但那是"内部清分",不是卡组织那种"网络清算"。**
