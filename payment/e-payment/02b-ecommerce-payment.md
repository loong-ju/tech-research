# 电商支付全景：电子支付的业务舞台（买家怎么付、卖家怎么收）

> **学习者**：AWS 技术架构师 · 支付小白
> **本篇定位（重要）**：电商（线上买卖）是**电子支付基础设施诞生的业务土壤/前置**——先有"在网上把东西卖给陌生人"的需求，才催生网关/第三方支付/钱包（模块2）。所以本篇有**双重身份**：① 作为 **`02-epayment-business.md` 的业务前置**，先建立"电商里钱怎么走"的语境；② 作为**模块1（卡四方模型/PayFac）+模块2（网关/钱包）机制的综合应用**，把买家付款方式、卖家收款方式逐一落到底层轨道。
> **本篇目标**：把"电商场景里一笔钱怎么走"讲透——**买家侧有哪些支付方式、卖家侧有哪些收款方式、各自底层走什么引擎、是推还是拉、信息流/资金流如何分离、谁担风险**。
> **阅读建议**：可作为模块2 的**业务入口先读**（建立电商语境）；其中涉及四方模型/PayFac/网关/钱包的机制细节，可**与 `01-cards-business.md` + `02-epayment-business.md` 并行参照**（本篇侧重"场景里怎么用"，那两篇侧重"机制本身"）。企业画像见 `02c-epayment-players/stripe.md`、`03c-crossborder-players/`。
...
| 资金底座 | 本质 | 典型代表 | 走什么轨道 | 归类依据 |
|---|---|---|---|---|
| **(a) 保存的支付凭证** | 钱包只是把卡/银行"包/存了一层"，付款时用底层资金源、自己不持余额 | **Apple Pay、Google Pay、Shop Pay、Stripe Link** | **卡/银行轨道**（同卡费率/同资金源） | 📌 Apple Pay/Shop Pay/Link 一手确证 |
| **(b) 独立钱包余额（储值）** | 钱可存在钱包公司账户里、能直接花用余额，不足时再扣绑定资金源（混合） | **PayPal、Cash App** | 钱包账户体系（账户型储值） | 📌 SEC/官方 ToS 确证 |
| （重定向账户型） | 账户内已存支付信息、付款走重定向 | **Amazon Pay** | 账户型（§2.4②、§4.3） | 📌 Stripe docs |

📌 **可信度升级（已核查）**：
- 📌 **PayPal、Cash App = 真·储值余额账户**（可持有并花用余额；Cash App 混合=余额优先、不足时扣 Linked Accounts）——来源：PayPal Help、Cash App ToS、Block 10-K（附A[27]）。
- 📌 **Shop Pay、Stripe Link = "加速结账/保存支付信息"，骑在卡/银行轨道、不持任何储值余额**——Stripe Link 官方："不论资金来源，都按卡支付的时间线处理进 Stripe 余额"；Shop Pay 接受标准卡、不持资金（附A[28][29]）。
- ⚠️ **被核查否决（0-3）**："按 Stripe 是否需重定向来推断走卡轨道 vs 账户型"**不成立**（Link 无重定向但仍可对接银行账户）——**不能用"要不要重定向"反推资金底座**。
...
| BNPL（Klarna/Afterpay/Affirm） | 第三方信贷垫付 | 特殊（§6.2） | **BNPL 公司担**（放贷方，下单即垫付全额） | 📌 招股书确证 |
...
### 6.2 BNPL：第三方垫付、消费者分期 📌

📌 **机制（已核查，三家招股书/10-K 一致：Affirm S-1/10-K、Klarna 招股书、Block 10-K，附A[24][25][26]）：**
- 📌 **谁先垫付**：BNPL 公司在**消费者下单时即向商户一次性垫付全额货款**（扣除百分比手续费），随后向消费者**分期收款**。Block 10-K 原文："pay retail merchants the full order value up front (less a percentage fee) and assume the risk of non-payment from the consumer"。
- 📌 **谁担风险**：**消费者违约/信用风险由 BNPL 公司承担**——它是**放贷方**，不是商户（Affirm：自留贷款 "we bear the entire credit risk in the event of consumer default"）。
- 📌 **放款结构**：常是"**银行合作方放款 + BNPL 公司买入债权**"——如 Affirm 多数美国贷款由合作银行（FY2025 为 Celtic Bank、Lead Bank；IPO 期为 Cross River Bank）发起、Affirm 再按"本金+费用+应计利息"买入（也可凭自有牌照在美/加/英直接放款）。
- 📌 **商户费率为何更高（机制）**：商户费**按个案协商、随产品/消费者信用/融资期限变动，0% APR 免息分期产品商户费更高**（Affirm 10-K）——商户用更高费率换 BNPL 带来的转化率/客单价提升。
- ⚠️ **MDR 具体数字区间未核实**：一手 SEC 文件**只确证机制、未披露 MDR 百分比区间**；网传"BNPL MDR 约 2%-8%、远高于卡交换费"属二手/营销口径，**写正式材料须另核一手**（商户定价页/费率表），勿当事实引用。
...
> 🔗 **业务↔技术呼应**：这里讲的是信息流/资金流分离的**业务现象**；它在**工程上为什么必须异步**（线上不能同步等结果、要靠异步回调+主动查询兜底）、**支付状态机**怎么设计（支付中→待确认→成功/失败），见技术篇 **`02-epayment-tech-aws.md §2.2（同步授权 vs 异步通知）/§2.3（支付状态机）**——同一回事的业务面与工程面。
...
- **本篇回应的"基础设施"本身**（网关/第三方支付/钱包/二维码/聚合支付）→ `02-epayment-business.md`（本篇是它的业务舞台/前置，那篇讲这些工具的机制）
...
| [16] | CFPB — BNPL Market Trends 2022 | Primary | BNPL 市场背景 |
| [17] | Affirm/Afterpay — How it works / business | Primary | BNPL 商户侧 |
| [24] | Affirm — S-1（2020）+ 10-K（FYE2025）SEC | Primary | BNPL 垫付全额/自担信用风险/银行合作放款买债权/0%APR商户费更高 |
| [25] | Klarna — 424(b)(4) 招股书 SEC | Primary | 下单时代消费者付商户/信用风险为核心 |
| [26] | Block — 10-K（FY2024）SEC | Primary | Afterpay 全额垫付+assume non-payment risk |
...
**Q：BNPL 商户为什么愿意付更高费率？**
A：📌 BNPL 公司**下单时即垫付全额给商户、自担消费者违约风险**（招股书确证），商户用更高 MDR 换转化/客单价提升；0% APR 免息产品商户费更高（§6.2）。⚠️ MDR 具体百分比区间一手文件未披露，勿引二手数字。
