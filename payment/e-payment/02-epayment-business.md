...
> 🌱 **谁催生了这两个新问题？——电商（电子支付的业务土壤/前置）**：上面"线上没 POS、陌生人不见面"这两个问题，不是凭空冒出来的——是**电子商务（线上买卖）这个业务场景**逼出来的：先有"想在网上把东西卖给陌生人"的需求，才需要网关来受理、需要第三方/担保交易来补信任。**所以本模块的所有基础设施，本质都是在回应电商的支付需求。** 想先建立"电商支付全景（买家怎么付、卖家怎么收、有哪些场景）"的业务语境，可先看 **`02b-ecommerce-payment.md`**；它既是本模块的**业务前置**，也是模块1+2 机制的**综合应用**，与本篇可并行阅读、互为表里。
...
> ⚠️ **别被"账户+二维码"简化误导：中国第三方支付一样有完整的网关/收单逻辑，受理也不止二维码。** 上图为突出"绕开卡组织"做了简化，要厘清三点：
> - **受理 ≠ 只有二维码**：还有 **POS/智能POS、App/H5/小程序、刷脸、NFC**……二维码只是其中**零硬件**那个入口（银联商务"全民付"就是多入口）。
> - **网关/收单一直都在**：商户怎么接入、交易怎么受理路由、谁给商户做入网(KYB)与资金结算——这层由**持牌收单机构 / 聚合支付服务商**承担，**断直连没有取消它**（详见 §5.2.1 收银台/网关/收单三角色）。
> - **"闭环"指的是清算账本那一层，不是"没有网关收单"**：① **余额→余额**（买卖双方都在同一钱包有账户）那笔在自家账本完成、**不经网联**（§4.1、本节下文）；② 但**只要碰银行**（充值/提现/绑卡卡扣/给没开余额的商户结算）就**必经网联**（断直连 2018 的硬规定，§4.2.1）——这部分**有完整的网关受理+收单+网联清算**。
> - 🔑 **断直连改的是"清算路由"（第三方直连银行 → 必经网联/银联），不是取消网关/收单**。完整链路其实是：**受理(多入口) → 网关 → 收单 → [余额闭环 或 网联清算]**。所以"中国=账户+二维码闭环"是对**清算账本层**的概括，**不代表中国支付没有网关/收单**。
...
| **欧盟/英国** | **EMI（电子货币机构）**——发行电子货币+持有客户余额的专门牌照，一张经 passporting 覆盖全 EEA | CBI/CSSF/DNB/FCA（见 `支付术语速查.md`） |
...
> ⚠️ **可信度**：牌照触发点（持余额=储值/EMI/MTL）、PCI 触发点（碰卡=PCI L1）、缩 scope 策略，均为本文件 §4.2.1 + `支付术语速查.md` 中**已核查/确定规则**（🔧 公知级 + 📌 牌照已核查）；"纯余额完全免 PCI"属边界推断，实务中极少有钱包完全不支持卡充值，写正式材料请按具体产品形态核 PCI-DSS SAQ 适用类型。
